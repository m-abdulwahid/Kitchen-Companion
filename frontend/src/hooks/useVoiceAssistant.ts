...
/**
 * Ella's voice, both directions, one audio player.
 * - toggle(step): tap-to-talk. First call starts recording, second sends it; the spoken
 *   reply plays automatically. Calling it while Ella is talking interrupts and records.
 * - speak(text, upcoming?): Ella reads text aloud (recipe steps, feedback). Ignored while the
...
export function useVoiceAssistant({ onReply, onError, onSpoken, companion }: Options) {
  ...
  /** Stop anything Ella is saying or about to say. */
  ...
  /** Pass the current recipe step so Ella answers in context. */
